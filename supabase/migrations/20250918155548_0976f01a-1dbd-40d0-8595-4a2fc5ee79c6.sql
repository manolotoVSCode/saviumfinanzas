-- Remove 'Reembolso' from categorias tipo constraint
ALTER TABLE public.categorias DROP CONSTRAINT IF EXISTS categorias_tipo_check;

-- Add the constraint back without 'Reembolso'
...
-- Delete any existing 'Reembolso' categories
DELETE FROM public.categorias WHERE tipo = 'Reembolso';

-- Update sample data function to remove reembolso categories
...
BEGIN
  ...
  -- Insert sample categories without reembolso types
  ...
  -- Insert sample transactions (last 30 days) without reembolso transactions
